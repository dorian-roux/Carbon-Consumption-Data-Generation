...
class Data_Generation: #Define the Point of Sales Class
    ...
    def generate_time_data(self, current_daytime):
        year = current_daytime.year
        month = current_daytime.month
        day = current_daytime.day
        weekday = self.weekdays[current_daytime.weekday()]
        week_numb = int(current_daytime.strftime("%V"))
        time_hm = current_daytime.time()
        date = datetime.datetime(year, month, day)

        return date, year, month, day, weekday, week_numb, time_hm
    ...
    def Generation(self):
        ...
        self.test["Date"] = pd.to_datetime(self.test["Full Date"]).dt.date
        quantile_lst = list(self.test.groupby(["Employee Daily Consumption Level", "Date"])["Daily Consumption"].max().reset_index(name = "Max Consumption")["Max Consumption"].quantile([0.25, 0.5, 0.75]).reset_index(name = "Quantile")["Quantile"])
        quantile_lst

        temp_df = self.test.groupby(["Employee ID", "Date"])["Daily Consumption"].max().reset_index(name = "Daily Consumption")
        ...
        self.test = self.test.merge(temp_df[["Employee ID", "Date", "True Employee Daily Consumption Level"]], how='left', on=['Employee ID', 'Date'])


    def Generate_Time_df(self):
        dict_time = dict({"Full Date":[], "Date": [], "Year":[], "Month":[], "Day":[], "Weekday":[], "Week Number":[], "Time":[]})
        for fdate in self.test['Full Date']:
